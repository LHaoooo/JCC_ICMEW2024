...
from absa.eval_metric import eval_absa, eval_masc, eval_mate
...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if args.task == 'absa' or args.task == 'mate':
            batch_start_logits, batch_end_logits, _ = model('extraction', input_mask, input_ids=input_ids,\
                token_type_ids=segment_ids,image_labels=image_labels,image_raw_data=image_raw_data)
        elif args.task == 'masc':
            batch_start_logits, batch_end_logits = start_positions, end_positions
        ...
        if args.task == 'absa':
            ...
            loss = model('train_mate', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                        start_positions=start_positions, end_positions=end_positions,
                        span_starts=span_starts, span_ends=span_ends,
                        polarity_labels=labels, label_masks=label_masks,
                        image_labels=image_labels,image_raw_data=image_raw_data)
        elif args.task == 'masc':
            loss = model('train_masc', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                        start_positions=start_positions, end_positions=end_positions,
                        span_starts=span_starts, span_ends=span_ends,
                        polarity_labels=labels, label_masks=label_masks,
                        image_labels=image_labels,image_raw_data=image_raw_data)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % save_checkpoints_steps == 0 and global_step > start_save_steps and count != 0:  # eval & save model
                ...
                if args.task == 'absa':
                    ...
                    if mate_metrics['mate_f1'] > best_f1:
                        best_f1 = mate_metrics['mate_f1']
                        torch.save({
                            'model': model.state_dict(),
                            'optimizer': optimizer.state_dict(),
                            'step': global_step
                    ...
                    # write masc
                    f = open(log_path, "a")
                    print("masc dev step: {}, loss: {:.4f}, MASC_acc: {:.4f}, MASC_F1: {:.4f} (MASC_retrieved: {}, MASC_relevant: {})"
                        .format(global_step, running_loss / count, masc_metrics['masc_acc'], masc_metrics['masc_f1'], 
                                masc_metrics['masc_retrieved'], masc_metrics['masc_relevant']), file=f)
                    f = open(log_path, "a")
                    print("masc test step: {}, loss: {:.4f}, MASC_acc: {:.4f}, MASC_F1: {:.4f} (MASC_retrieved: {}, MASC_relevant: {})"
                        .format(global_step, running_loss / count, masc_metrics_test['masc_acc'], masc_metrics_test['masc_f1'], 
                                masc_metrics_test['masc_retrieved'], masc_metrics_test['masc_relevant']), file=f)
                    print(' ', file=f)
                    f.close()
                    ...
                    if masc_metrics['masc_f1'] > best_f1:
                        best_f1 = masc_metrics['masc_f1']
                        torch.save({
                            'model': model.state_dict(),
                            'optimizer': optimizer.state_dict(),
                            'step': global_step
...
def evaluate(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            batch_start_logits, batch_end_logits, sequence_output = model('extraction', input_mask,
                                                                          input_ids=input_ids,
        ...
        with torch.no_grad():
            batch_ac_logits= model('classification', input_mask, input_ids=input_ids,
...
def evaluate_masc(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    if write_pred:
        output_file = os.path.join(args.output_dir, "MASC_predictions.json")
        with open(output_file, "w") as writer:
            writer.write(json.dumps(all_nbest_json, indent=4) + "\n")
        logger.info("Writing predictions to: %s" % (output_file))
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--bert_config_file", default='../data/bert/bert-base-uncased/bert_config.json', 
                        type=str, required=False,
                        help="The config json file corresponding to the pre-trained BERT model. "
                             "This specifies the model architecture.")
    parser.add_argument("--vocab_file", default='../data/bert/bert-base-uncased/vocab.txt', 
                        type=str, required=False,
                        help="The vocabulary file that the BERT model was trained on.")
    parser.add_argument("--output_dir", default='../debug', type=str, required=False,
                        help="The output directory where the model checkpoints will be written.")
    parser.add_argument("--data_dir", default='../data/Twitter/twitter15_pre', type=str, help="SemEval data dir")
    parser.add_argument("--train_file", default=None, type=str, help="Twitter text dataset for training")
    parser.add_argument("--predict_file", default=None, type=str, help="Twitter text dataset for prediction")
    parser.add_argument("--init_checkpoint", default='../data/bert/bert-base-uncased/pytorch_model.bin', type=str,
    ...
    parser.add_argument("--task", default='absa', required=False, type = str, help="choose the task mode: absa\mate\masc.")
    parser.add_argument("--text_encoder", default='bert',type=str, help='utilize which model to extract text featrue')
    ...
    if args.text_encoder == 'roberta':
        ...
        tokenizer = Tokenizer.from_file(args.token_file)
    ...
    model = JointMMwithRel(args.text_encoder, bert_config)
    if args.init_checkpoint is not None and not os.path.isfile(save_path):
        if args.text_encoder == 'bert':
            ...
            text_pretrained_dict = RobertaForTokenClassification.from_pretrained(args.init_checkpoint).state_dict()
            roberta_model = model.bert
            roberta_model_dict = roberta_model.state_dict()
            for k,v in text_pretrained_dict.items():
                if roberta_model_dict.get(k) is not None and k not in {'classifier.bias', 'classifier.weight'}:
                    roberta_model_dict[k] = v
            roberta_model.load_state_dict(roberta_model_dict)
    ...
    if args.do_predict:
        ...
        if args.task == 'absa':
            ...
            print(masc_metrics)
            f = open(log_path, "a")
            print("threshold: {}, step: {}, masc_acc: {:.4f}, masc_F1: {:.4f} (masc_retrieved: {}, masc_relevant: {})"
